...
struct EnclosingCircle {
   ...
   */
   ...
   func setUpLayer() {
      ...
      let circlePath: UIBezierPath = UIBezierPath(roundedRect: CGRect(x: self.view.frame.width / 2 - self.radius, y: self.view.frame.height / 2 - self.radius, width: self.radius * 2, height: self.radius * 2), cornerRadius: self.radius)
   ...
   func animate() {
      ...
      // Create the path for the layer
      let path: UIBezierPath = UIBezierPath(roundedRect: CGRect(origin: CGPoint.zero, size: CGSize(width: self.view.frame.width, height: self.view.frame.height)), cornerRadius: 0)
      let circlePath: UIBezierPath = UIBezierPath(roundedRect: CGRect(x: view.frame.width / 2 - radius, y: view.frame.height / 2 - radius, width: radius * 2, height: radius * 2), cornerRadius: radius)
      path.appendPath(circlePath)
